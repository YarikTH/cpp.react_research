...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename... TArgs> inline void pass(TArgs &&...) {}

template <typename T> bool IsBitmaskSet(T flags, T mask) {
...
/// API constants
///////////////////////////////////////////////////////////////////////////////////////////////////
enum class WeightHint { automatic, light, heavy };
...
// Ref
template <typename T> using Ref = std::reference_wrapper<const T>;

// State
template <typename S> class State;

template <typename S> class StateVar;

template <typename S> class StateSlot;

template <typename S> class StateLink;

template <typename S> using StateRef = State<Ref<S>>;

// Event
enum class Token;

template <typename E = Token> class Event;

template <typename E = Token> class EventSource;

template <typename E = Token> class EventSlot;

template <typename E = Token> using EventValueList = std::vector<E>;
...
template <typename T> bool HasChanged(const T &a, const T &b) {
  return !(a == b);
}

template <typename T> bool HasChanged(const Ref<T> &a, const Ref<T> &b) {
  return true;
}

template <typename T, typename V> void ListInsert(T &list, V &&value) {
  list.push_back(std::forward<V>(value));
}

template <typename T, typename V> void MapInsert(T &map, V &&value) {
  map.insert(std::forward<V>(value));
}

///////////////////////////////////////////////////////////////////////////////////////////////////
/// A synchronization primitive that is essentially a cooperative semaphore,
/// i.e. it blocks a consumer until all producers are done. Usage:
///     SyncPoint sp;
///     // Add a dependency the sync point should wait for. The depenency is
///     released on destruction. SyncPoint::Depenency dep(sp);
///     // Pass dependency to some operation. Dependencies can be copied as
///     well. SomeAsyncOperation(std::move(dep));
...
private:
  ...
  public:
    virtual void IncrementWaitCount() override { // mutex_
      std::lock_guard<std::mutex> scopedLock(mtx_);
      ++waitCount_;
    } // ~mutex_

    virtual void DecrementWaitCount() override { // mutex_
      std::lock_guard<std::mutex> scopedLock(mtx_);
      --waitCount_;
...
public:
  ...
  /// Blocks the calling thread until all dependencies of this sync point are
  /// released.
  void Wait() { state_->Wait(); }

  /// Like Wait, but times out after relTime. Returns false if the timeout was
  /// hit.
  template <typename TRep, typename TPeriod>
  bool WaitFor(const std::chrono::duration<TRep, TPeriod> &relTime) {
  ...
  public:
    ...
    explicit Dependency(const SyncPoint &sp) : target_(sp.state_) {
      target_->IncrementWaitCount();
    }

    /// Merges an input range of other dependencies into a single dependency.
    /// This allows to create APIs that are agnostic of how many dependent
    /// operations they process.
    ...
    Dependency(TBegin first, TEnd last) {
      ...
      if (count == 1) {
        ...
        for (; !(first == last); ++first)
          if (first->target_) // There's no point in propagating released/empty
                              // dependencies.
    ...
    Dependency(const Dependency &other) : target_(other.target_) {
      if (target_)
        target_->IncrementWaitCount();
    }
    ...
    Dependency(Dependency &&other) : target_(std::move(other.target_)) {}
    ...
    bool IsReleased() const { return target_ == nullptr; }
...
enum class UpdateResult { unchanged, changed, shifted };

enum class NodeCategory { normal, input, dyninput, output, linkoutput };

class ReactGraph;
struct IReactNode;

using LinkOutputMap =
    std::unordered_map<ReactGraph *, std::vector<std::function<void()>>>;
...
/// Thread-safe.
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename K, typename V> class WeakPtrCache {
public:
  /// Returns a shared pointer to an object that existings in the cache, indexed
  /// by key. If no hit was found, createFunc is used to construct the object
  /// managed by shared pointer. A weak pointer to the result is stored in the
  /// cache.
  template <typename F>
  std::shared_ptr<V> LookupOrCreate(const K &key, F &&createFunc) {
    ...
    if (it != map_.end()) {
      // Lock fails, if the object was cached before, but has been released
      // already. In that case we re-create it.
...
///////////////////////////////////////////////////////////////////////////////////////////////////
/// A simple slot map.
/// Insert returns the slot index, which stays valid until the element is
/// erased.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
public:
  ...
  T &operator[](size_t index) { return reinterpret_cast<T &>(data_[index]); }
  ...
  void Erase(size_t index) {
    // If we erased something other than the last element, save in free index
    // list.
    if (index != (size_ - 1)) {
...
private:
  T &GetDataAt(size_t index) { return reinterpret_cast<T &>(data_[index]); }

  T &GetDataAt(size_t index) const {
    return reinterpret_cast<T &>(data_[index]);
  }

  bool IsAtFullCapacity() const { return capacity_ == size_; }

  bool HasFreeIndices() const { return freeSize_ > 0; }
  ...
  void Grow() {
    ...
    for (size_t i = 0; i < capacity_; ++i) {
      new (reinterpret_cast<T *>(&newData[i]))
          T{std::move(reinterpret_cast<T &>(data_[i]))};
...
public:
  TransactionQueue(ReactGraph &graph) : graph_(graph) {}

  template <typename F>
  void Push(F &&func, SyncPoint::Dependency dep, TransactionFlags flags) {
    transactions_.push(
        StoredTransaction{std::forward<F>(func), std::move(dep), flags});
...
private:
  ...
  public:
    WorkerTask(TransactionQueue &parent) : parent_(parent) {}
...
public:
  ...
  template <typename F> void PushInput(NodeId nodeId, F &&inputCallback);

  void AddSyncPointDependency(SyncPoint::Dependency dep, bool syncLinked);

  void AllowLinkedTransactionMerging(bool allowMerging);

  template <typename F> void DoTransaction(F &&transactionCallback);

  template <typename F>
  void EnqueueTransaction(F &&func, SyncPoint::Dependency dep,
                          TransactionFlags flags);

  LinkCache &GetLinkCache() { return linkCache_; }
...
private:
  struct NodeData {
    ...
    NodeData(IReactNode *nodePtrIn, NodeCategory categoryIn)
        : nodePtr(nodePtrIn), category(categoryIn) {}
  ...
  public:
    ...
    const std::vector<NodeId> &Next() const { return nextData_; }

    bool IsEmpty() const { return queueData_.empty(); }
...
template <typename F> void ReactGraph::DoTransaction(F &&transactionCallback) {
...
template <typename F>
void ReactGraph::EnqueueTransaction(F &&func, SyncPoint::Dependency dep,
                                    TransactionFlags flags) {
...
public:
  GroupInternals() : graphPtr_(std::make_shared<ReactGraph>()) {}
  ...
  auto GetGraphPtr() -> std::shared_ptr<ReactGraph> & { return graphPtr_; }
...
/// Group
///////////////////////////////////////////////////////////////////////////////////////////////////
class Group : protected ::react::impl::GroupInternals {
public:
  Group() = default;
  ...
  template <typename F> void DoTransaction(F &&func) {
  ...
  void EnqueueTransaction(F &&func,
    ...
    GetGraphPtr()->EnqueueTransaction(std::forward<F>(func),
                                      SyncPoint::Dependency{}, flags);
  }

  template <typename F>
  void EnqueueTransaction(F &&func, const SyncPoint &syncPoint,
                          TransactionFlags flags = TransactionFlags::none) {
    GetGraphPtr()->EnqueueTransaction(std::forward<F>(func),
                                      SyncPoint::Dependency{syncPoint}, flags);
  }
  ...
  friend auto GetInternals(Group &g) -> ::react::impl::GroupInternals & {
    return g;
  }

  friend auto GetInternals(const Group &g)
      -> const ::react::impl::GroupInternals & {
    return g;
  }
};
...
public:
  NodeBase(const Group &group) : group_(group) {}
  ...
  auto GetGroup() const -> const Group & { return group_; }

  auto GetGroup() -> Group & { return group_; }
...
protected:
  ...
  void UnregisterMe() { GetGraphPtr()->UnregisterNode(nodeId_); }
...
/// StateNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateNode : public NodeBase {
public:
  explicit StateNode(const Group &group)
      : StateNode::NodeBase(group), value_() {}

  template <typename T>
  StateNode(const Group &group, T &&value)
      : StateNode::NodeBase(group), value_(std::forward<T>(value)) {}

  template <typename... Ts>
  StateNode(InPlaceTag, const Group &group, Ts &&...args)
      : StateNode::NodeBase(group), value_(std::forward<Ts>(args)...) {}
...
/// VarNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateVarNode : public StateNode<S> {
public:
  explicit StateVarNode(const Group &group)
      : StateVarNode::StateNode(group), newValue_() {
  ...
  ~StateVarNode() { this->UnregisterMe(); }
  ...
  template <typename T> void SetValue(T &&newValue) {
    ...
    // the only difference between the two is that isInputModified_
    // doesn't/can't compare
    isInputModified_ = false;
  }

  template <typename F> void ModifyValue(F &&func) {
    ...
    // The modified newValue will handled like before, i.e. it'll be compared to
    // value_ in ApplyInput
    else {
      func(newValue_);
    }
  }
...
public:
  ...
  StateFuncNode(const Group &group, FIn &&func, const State<TDeps> &...deps)
      : StateFuncNode::StateNode(group, func(GetInternals(deps).Value()...)),
        func_(std::forward<FIn>(func)), depHolder_(deps...) {
    this->RegisterMe();
  ...
  ~StateFuncNode() {
    react::impl::apply(
        [this](const auto &...deps) {
          REACT_EXPAND_PACK(
              this->DetachFromMe(GetInternals(deps).GetNodePtr()->GetNodeId()));
        },
        depHolder_);
    this->UnregisterMe();
  }
  ...
  virtual UpdateResult Update(TurnId turnId) noexcept override {
    S newValue = react::impl::apply(
        [this](const auto &...deps) {
          return this->func_(GetInternals(deps).Value()...);
        },
        depHolder_);
...
/// StateSlotNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateSlotNode : public StateNode<S> {
public:
  StateSlotNode(const Group &group, const State<S> &dep)
      : StateSlotNode::StateNode(group, GetInternals(dep).Value()),
        input_(dep) {
    inputNodeId_ =
        this->GetGraphPtr()->RegisterNode(&slotInput_, NodeCategory::dyninput);
  ...
  NodeId GetInputNodeId() const { return inputNodeId_; }
...
/// StateLinkNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateLinkNode : public StateNode<S> {
public:
  StateLinkNode(const Group &group, const State<S> &dep)
      : StateLinkNode::StateNode(group, GetInternals(dep).Value()), dep_(dep),
        srcGroup_(dep.GetGroup()) {
    this->RegisterMe(NodeCategory::input);

    auto &srcGraphPtr = GetInternals(srcGroup_).GetGraphPtr();
    outputNodeId_ =
        srcGraphPtr->RegisterNode(&linkOutput_, NodeCategory::linkoutput);
  ...
  void SetValue(S &&newValue) { this->Value() = std::move(newValue); }
    ...
    virtual void CollectOutput(LinkOutputMap &output) override {
      if (auto p = parent.lock()) {
        ...
        output[rawPtr].push_back(
            [storedParent = std::move(p),
             storedValue = GetInternals(p->dep_).Value()]() mutable -> void {
...
/// StateInternals
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateInternals {
public:
  ...
  auto GetNodePtr() -> std::shared_ptr<StateNode<S>> & { return nodePtr_; }

  auto GetNodePtr() const -> const std::shared_ptr<StateNode<S>> & {
    return nodePtr_;
  }

  NodeId GetNodeId() const { return nodePtr_->GetNodeId(); }

  S &Value() { return nodePtr_->Value(); }

  const S &Value() const { return nodePtr_->Value(); }
...
/// StateRefNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateRefNode : public StateNode<Ref<S>> {
public:
...
/// State
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class State : protected ::react::impl::StateInternals<S> {
public:
  // Construct with explicit group
  template <typename F, typename T1, typename... Ts>
  static State Create(const Group &group, F &&func, const State<T1> &dep1,
                      const State<Ts> &...deps) {
  ...
  template <typename F, typename T1, typename... Ts>
  static State Create(F &&func, const State<T1> &dep1,
                      const State<Ts> &...deps) {
    return State(
        CreateFuncNode(dep1.GetGroup(), std::forward<F>(func), dep1, deps...));
  }

  // Construct with constant value
  template <typename T> static State Create(const Group &group, T &&init) {
    return State(
        CreateFuncNode(group, [value = std::move(init)] { return value; }));
  }
  ...
  auto GetGroup() -> Group & { return this->GetNodePtr()->GetGroup(); }
  ...
  friend auto GetInternals(State<S> &s) -> ::react::impl::StateInternals<S> & {
    return s;
  }

  friend auto GetInternals(const State<S> &s)
      -> const ::react::impl::StateInternals<S> & {
    return s;
  }

protected:
  State(std::shared_ptr<::react::impl::StateNode<S>> &&nodePtr)
      : State::StateInternals(std::move(nodePtr)) {}

private:
  template <typename F, typename T1, typename... Ts>
  static auto CreateFuncNode(const Group &group, F &&func,
                             const State<T1> &dep1, const State<Ts> &...deps)
      -> decltype(auto) {
    using ::react::impl::SameGroupOrLink;
    using ::react::impl::StateFuncNode;

    return std::make_shared<
        StateFuncNode<S, typename std::decay<F>::type, T1, Ts...>>(
        group, std::forward<F>(func), SameGroupOrLink(group, dep1),
        SameGroupOrLink(group, deps)...);
  }
...
/// StateVar
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateVar : public State<S> {
public:
  ...
  template <typename T> static StateVar Create(const Group &group, T &&value) {
  ...
  void Set(const S &newValue) { SetValue(newValue); }

  void Set(S &&newValue) { SetValue(std::move(newValue)); }

  template <typename F> void Modify(const F &func) { ModifyValue(func); }
  ...
  S *operator->() { return &this->Value(); }

protected:
  StateVar(std::shared_ptr<::react::impl::StateNode<S>> &&nodePtr)
      : StateVar::State(std::move(nodePtr)) {}

private:
  static auto CreateVarNode(const Group &group) -> decltype(auto) {
    using ::react::impl::StateVarNode;
  ...
  template <typename T> void SetValue(T &&newValue) {
    using ::react::impl::NodeId;
    using VarNodeType = ::react::impl::StateVarNode<S>;

    VarNodeType *castedPtr =
        static_cast<VarNodeType *>(this->GetNodePtr().get());

    NodeId nodeId = castedPtr->GetNodeId();
    auto &graphPtr = GetInternals(this->GetGroup()).GetGraphPtr();

    graphPtr->PushInput(nodeId, [castedPtr, &newValue] {
      castedPtr->SetValue(std::forward<T>(newValue));
    });
  }

  template <typename F> void ModifyValue(const F &func) {
    using ::react::impl::NodeId;
    using VarNodeType = ::react::impl::StateVarNode<S>;
    ...
    graphPtr->PushInput(nodeId,
                        [castedPtr, &func] { castedPtr->ModifyValue(func); });
  }
};
...
/// StateSlotBase
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateSlot : public State<S> {
public:
  // Construct with explicit group
  ...
  void Set(const State<S> &newInput) { SetSlotInput(newInput); }

protected:
  StateSlot(std::shared_ptr<::react::impl::StateNode<S>> &&nodePtr)
      : StateSlot::State(std::move(nodePtr)) {}

private:
  static auto CreateSlotNode(const Group &group, const State<S> &input)
      -> decltype(auto) {
    using ::react::impl::SameGroupOrLink;
    using ::react::impl::StateSlotNode;

    return std::make_shared<StateSlotNode<S>>(group,
                                              SameGroupOrLink(group, input));
  }
  ...
  void SetSlotInput(const State<S> &newInput) {
    ...
    graphPtr->PushInput(nodeId, [this, castedPtr, &newInput] {
      castedPtr->SetInput(SameGroupOrLink(this->GetGroup(), newInput));
    });
  }
};
...
/// StateLink
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateLink : public State<S> {
public:
...
protected:
  StateLink(std::shared_ptr<::react::impl::StateNode<S>> &&nodePtr)
      : StateLink::State(std::move(nodePtr)) {}

private:
  static auto GetOrCreateLinkNode(const Group &group, const State<S> &input)
      -> decltype(auto) {
    using ::react::impl::IReactNode;
    using ::react::impl::ReactGraph;
    using ::react::impl::StateLinkNode;

    IReactNode *k = GetInternals(input).GetNodePtr().get();

    ReactGraph::LinkCache &linkCache =
        GetInternals(group).GetGraphPtr()->GetLinkCache();
...
/// CreateRef
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> auto CreateRef(const State<S> &state) -> State<Ref<S>> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::StateRefNode;

  return CreateWrappedNode<State<Ref<S>>, StateRefNode<S>>(state.GetGroup(),
                                                           state);
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class StateNode;

///////////////////////////////////////////////////////////////////////////////////////////////////
/// EventNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename E> class EventNode : public NodeBase {
public:
  explicit EventNode(const Group &group) : EventNode::NodeBase(group) {}

  EventValueList<E> &Events() { return events_; }

  const EventValueList<E> &Events() const { return events_; }

  virtual void Clear() noexcept override { events_.clear(); }
...
/// EventSourceNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename E> class EventSourceNode : public EventNode<E> {
public:
  EventSourceNode(const Group &group) : EventSourceNode::EventNode(group) {
    this->RegisterMe(NodeCategory::input);
  }

  ~EventSourceNode() { this->UnregisterMe(); }
  ...
  template <typename U> void EmitValue(U &&value) {
...
public:
  EventMergeNode(const Group &group, const Event<TInputs> &...deps)
      : EventMergeNode::EventNode(group), inputs_(deps...) {
    this->RegisterMe();
    REACT_EXPAND_PACK(this->AttachToMe(GetInternals(deps).GetNodeId()));
  }
  ...
  ~EventMergeNode() {
    react::impl::apply(
        [this](const auto &...inputs) {
          REACT_EXPAND_PACK(
              this->DetachFromMe(GetInternals(inputs).GetNodeId()));
        },
        inputs_);
    this->UnregisterMe();
  }
  ...
  virtual UpdateResult Update(TurnId turnId) noexcept override {
    react::impl::apply(
        [this](auto &...inputs) { REACT_EXPAND_PACK(MergeFromInput(inputs)); },
        inputs_);
...
private:
  template <typename U> void MergeFromInput(Event<U> &dep) {
    auto &depInternals = GetInternals(dep);
    this->Events().insert(this->Events().end(), depInternals.Events().begin(),
                          depInternals.Events().end());
  }
...
/// EventSlotNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename E> class EventSlotNode : public EventNode<E> {
public:
  EventSlotNode(const Group &group) : EventSlotNode::EventNode(group) {
    inputNodeId_ =
        this->GetGraphPtr()->RegisterNode(&slotInput_, NodeCategory::dyninput);
    this->RegisterMe();
...
public:
  ...
  EventProcessingNode(const Group &group, FIn &&func, const Event<TIn> &dep)
      : EventProcessingNode::EventNode(group), func_(std::forward<FIn>(func)),
        dep_(dep) {
    this->RegisterMe();
    this->AttachToMe(GetInternals(dep).GetNodeId());
  }
...
public:
  template <typename FIn>
  SyncedEventProcessingNode(const Group &group, FIn &&func,
                            const Event<TIn> &dep,
                            const State<TSyncs> &...syncs)
      : SyncedEventProcessingNode::EventNode(group),
        func_(std::forward<FIn>(func)), dep_(dep), syncHolder_(syncs...) {
    this->RegisterMe();
    this->AttachToMe(GetInternals(dep).GetNodeId());
  ...
  ~SyncedEventProcessingNode() {
    react::impl::apply(
        [this](const auto &...syncs) {
          REACT_EXPAND_PACK(
              this->DetachFromMe(GetInternals(syncs).GetNodeId()));
        },
        syncHolder_);
    this->DetachFromMe(GetInternals(dep_).GetNodeId());
    this->UnregisterMe();
...
public:
  EventJoinNode(const Group &group, const Event<Ts> &...deps)
      : EventJoinNode::EventNode(group), slots_(deps...) {
    this->RegisterMe();
    REACT_EXPAND_PACK(this->AttachToMe(GetInternals(deps).GetNodeId()));
  }
  ...
  ~EventJoinNode() {
    react::impl::apply(
        [this](const auto &...slots) {
          REACT_EXPAND_PACK(
              this->DetachFromMe(GetInternals(slots.source).GetNodeId()));
        },
        slots_);
    this->UnregisterMe();
  }
  ...
  virtual UpdateResult Update(TurnId turnId) noexcept override {
    ...
    react::impl::apply(
        [this, turnId](Slot<Ts> &...slots) {
          REACT_EXPAND_PACK(FetchBuffer(turnId, slots));
        },
        slots_);
...
private:
  template <typename U> struct Slot {
    Slot(const Event<U> &src) : source(src) {}

    Event<U> source;
    std::deque<U> buffer;
  };

  template <typename U> static void FetchBuffer(TurnId turnId, Slot<U> &slot) {
    slot.buffer.insert(slot.buffer.end(),
                       GetInternals(slot.source).Events().begin(),
                       GetInternals(slot.source).Events().end());
  }

  template <typename T> static void CheckSlot(Slot<T> &slot, bool &isReady) {
...
/// EventLinkNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename E> class EventLinkNode : public EventNode<E> {
public:
  EventLinkNode(const Group &group, const Event<E> &dep)
      : EventLinkNode::EventNode(group), dep_(dep), srcGroup_(dep.GetGroup()) {
    this->RegisterMe(NodeCategory::input);
  ...
  struct VirtualOutputNode : public IReactNode {
    ...
    virtual void CollectOutput(LinkOutputMap &output) override {
      if (auto p = parent.lock()) {
        ...
        output[rawPtr].push_back(
            [storedParent = std::move(p),
             storedEvents = GetInternals(p->dep_).Events()]() mutable {
              NodeId nodeId = storedParent->GetNodeId();
              auto &graphPtr = storedParent->GetGraphPtr();

              graphPtr->PushInput(nodeId, [&storedParent, &storedEvents] {
...
/// EventInternals
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename E> class EventInternals {
public:
  ...
  auto GetNodePtr() -> std::shared_ptr<EventNode<E>> & { return nodePtr_; }
  ...
  EventValueList<E> &Events() { return nodePtr_->Events(); }

  const EventValueList<E> &Events() const { return nodePtr_->Events(); }
...
/// Event
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename E> class Event : protected ::react::impl::EventInternals<E> {
public:
  // Construct with explicit group
  template <typename F, typename T>
  ...
  static Event Create(F &&func, const Event<T> &dep) {
    return Event(
        CreateProcessingNode(dep.GetGroup(), std::forward<F>(func), dep));
  }

  // Construct with explicit group
  template <typename F, typename T, typename... Us>
  static Event Create(const Group &group, F &&func, const Event<T> &dep,
                      const State<Us> &...states) {
    return Event(CreateSyncedProcessingNode(group, std::forward<F>(func), dep,
                                            states...));
  }
  ...
  static Event Create(F &&func, const Event<T> &dep,
                      const State<Us> &...states) {
    return Event(CreateSyncedProcessingNode(
        dep.GetGroup(), std::forward<F>(func), dep, states...));
  }
  ...
  friend auto GetInternals(Event<E> &e) -> ::react::impl::EventInternals<E> & {
    return e;
  }

  friend auto GetInternals(const Event<E> &e)
      -> const ::react::impl::EventInternals<E> & {
    return e;
  }

protected:
  Event(std::shared_ptr<::react::impl::EventNode<E>> &&nodePtr)
      : Event::EventInternals(std::move(nodePtr)) {}

  template <typename F, typename T>
  static auto CreateProcessingNode(const Group &group, F &&func,
                                   const Event<T> &dep) -> decltype(auto) {
    using ::react::impl::EventProcessingNode;
    using ::react::impl::SameGroupOrLink;

    return std::make_shared<
        EventProcessingNode<E, T, typename std::decay<F>::type>>(
        group, std::forward<F>(func), SameGroupOrLink(group, dep));
  }

  template <typename F, typename T, typename... Us>
  static auto CreateSyncedProcessingNode(const Group &group, F &&func,
                                         const Event<T> &dep,
                                         const State<Us> &...syncs)
      -> decltype(auto) {
    using ::react::impl::SameGroupOrLink;
    using ::react::impl::SyncedEventProcessingNode;

    return std::make_shared<
        SyncedEventProcessingNode<E, T, typename std::decay<F>::type, Us...>>(
        group, std::forward<F>(func), SameGroupOrLink(group, dep),
        SameGroupOrLink(group, syncs)...);
  }
...
/// EventSource
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename E> class EventSource : public Event<E> {
public:
  ...
  void Emit(const E &value) { EmitValue(value); }

  void Emit(E &&value) { EmitValue(std::move(value)); }

  template <typename T = E,
            typename = std::enable_if_t<std::is_same_v<T, Token>>>
  void Emit() {
...
protected:
  EventSource(std::shared_ptr<::react::impl::EventNode<E>> &&nodePtr)
...
private:
  ...
  template <typename T> void EmitValue(T &&value) {
    ...
    auto *castedPtr =
        static_cast<EventSourceNode<E> *>(this->GetNodePtr().get());

    NodeId nodeId = castedPtr->GetNodeId();
    auto &graphPtr = GetInternals(this->GetGroup()).GetGraphPtr();

    graphPtr->PushInput(nodeId, [castedPtr, &value] {
...
/// EventSlotBase
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename E> class EventSlot : public Event<E> {
public:
  ...
  void Add(const Event<E> &input) { AddSlotInput(input); }

  void Remove(const Event<E> &input) { RemoveSlotInput(input); }

  void RemoveAll() { RemoveAllSlotInputs(); }

protected:
  EventSlot(std::shared_ptr<::react::impl::EventNode<E>> &&nodePtr)
      : EventSlot::Event(std::move(nodePtr)) {}

private:
  static auto CreateSlotNode(const Group &group) -> decltype(auto) {
    using ::react::impl::EventSlotNode;
    return std::make_shared<EventSlotNode<E>>(group);
  }

  void AddSlotInput(const Event<E> &input) {
    using ::react::impl::NodeId;
    using SlotNodeType = ::react::impl::EventSlotNode<E>;

    SlotNodeType *castedPtr =
        static_cast<SlotNodeType *>(this->GetNodePtr().get());

    NodeId nodeId = castedPtr->GetInputNodeId();
    auto &graphPtr = GetInternals(this->GetGroup()).GetGraphPtr();

    graphPtr->PushInput(nodeId, [this, castedPtr, &input] {
      castedPtr->AddSlotInput(SameGroupOrLink(this->GetGroup(), input));
    });
  }
  ...
  void RemoveSlotInput(const Event<E> &input) {
    ...
    graphPtr->PushInput(nodeId, [this, castedPtr, &input] {
      castedPtr->RemoveSlotInput(SameGroupOrLink(this->GetGroup(), input));
    });
  }
...
/// EventLink
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename E> class EventLink : public Event<E> {
public:
  // Construct with group
...
protected:
  EventLink(std::shared_ptr<::react::impl::EventNode<E>> &&nodePtr)
      : EventLink::Event(std::move(nodePtr)) {}

private:
  static auto GetOrCreateLinkNode(const Group &group, const Event<E> &input)
      -> decltype(auto) {
    using ::react::impl::EventLinkNode;
    using ::react::impl::IReactNode;
    using ::react::impl::ReactGraph;
...
static auto Merge(const Group &group, const Event<E> &dep1,
                  const Event<Us> &...deps) -> Event<E> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::EventMergeNode;
  using ::react::impl::SameGroupOrLink;
...
static auto Merge(const Event<U1> &dep1, const Event<Us> &...deps)
    -> decltype(auto) {
...
static auto Filter(const Group &group, F &&pred, const Event<E> &dep)
    -> Event<E> {
  auto filterFunc = [capturedPred =
                         std::forward<F>(pred)](const EventValueList<E> &events,
                                                EventValueSink<E> out) {
...
static auto Filter(const Group &group, F &&pred, const Event<E> &dep,
                   const State<Ts> &...states) -> Event<E> {
  auto filterFunc = [capturedPred = std::forward<F>(pred)](
                        const EventValueList<E> &evts, EventValueSink<E> out,
                        const Ts &...values) {
...
template <typename F, typename E, typename... Ts>
static auto Filter(F &&pred, const Event<E> &dep, const State<Ts> &...states)
    -> Event<E> {
...
static auto Transform(const Group &group, F &&op, const Event<T> &dep)
    -> Event<E> {
  auto transformFunc = [capturedOp =
                            std::forward<F>(op)](const EventValueList<T> &evts,
                                                 EventValueSink<E> out) {
...
static auto Transform(const Group &group, F &&op, const Event<T> &dep,
                      const State<Us> &...states) -> Event<E> {
  auto transformFunc = [capturedOp = std::forward<F>(op)](
                           const EventValueList<T> &evts, EventValueSink<E> out,
                           const Us &...values) {
    for (const auto &v : evts)
...
template <typename E, typename F, typename T, typename... Us>
static auto Transform(F &&op, const Event<T> &dep, const State<Us> &...states)
    -> Event<E> {
...
static auto Join(const Group &group, const Event<U1> &dep1,
                 const Event<Us> &...deps) -> Event<std::tuple<U1, Us...>> {
  using ::react::impl::CreateWrappedNode;
  ...
  return CreateWrappedNode<Event<std::tuple<U1, Us...>>,
                           EventJoinNode<U1, Us...>>(
      group, SameGroupOrLink(group, dep1), SameGroupOrLink(group, deps)...);
}

template <typename U1, typename... Us>
static auto Join(const Event<U1> &dep1, const Event<Us> &...deps)
    -> Event<std::tuple<U1, Us...>> {
...
public:
  ...
  IterateNode(const Group &group, T &&init, FIn &&func, const Event<E> &evnt)
      : IterateNode::StateNode(group, std::forward<T>(init)),
        func_(std::forward<FIn>(func)), evnt_(evnt) {
    this->RegisterMe();
...
public:
  template <typename T, typename FIn>
  IterateByRefNode(const Group &group, T &&init, FIn &&func,
                   const Event<E> &evnt)
...
public:
  template <typename T, typename FIn>
  SyncedIterateNode(const Group &group, T &&init, FIn &&func,
                    const Event<E> &evnt, const State<TSyncs> &...syncs)
      : SyncedIterateNode::StateNode(group, std::forward<T>(init)),
        func_(std::forward<FIn>(func)), evnt_(evnt), syncHolder_(syncs...) {
    this->RegisterMe();
    this->AttachToMe(GetInternals(evnt).GetNodeId());
    REACT_EXPAND_PACK(this->AttachToMe(GetInternals(syncs).GetNodeId()));
  }
  ...
  virtual UpdateResult Update(TurnId turnId) noexcept override {
    ...
    S newValue = react::impl::apply(
        [this](const auto &...syncs) {
          return func_(GetInternals(evnt_).Events(), this->Value(),
                       GetInternals(syncs).Value()...);
        },
        syncHolder_);
...
public:
  template <typename T, typename FIn>
  SyncedIterateByRefNode(const Group &group, T &&init, FIn &&func,
                         const Event<E> &evnt, const State<TSyncs> &...syncs)
...
/// HoldNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class HoldNode : public StateNode<S> {
public:
  template <typename T>
  HoldNode(const Group &group, T &&init, const Event<S> &evnt)
      : HoldNode::StateNode(group, std::forward<T>(init)), evnt_(evnt) {
    this->RegisterMe();
    this->AttachToMe(GetInternals(evnt).GetNodeId());
  }
...
/// SnapshotNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S, typename E> class SnapshotNode : public StateNode<S> {
public:
  SnapshotNode(const Group &group, const State<S> &target,
               const Event<E> &trigger)
      : SnapshotNode::StateNode(group, GetInternals(target).Value()),
        target_(target), trigger_(trigger) {
    this->RegisterMe();
...
/// MonitorNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S> class MonitorNode : public EventNode<S> {
public:
  MonitorNode(const Group &group, const State<S> &input)
      : MonitorNode::EventNode(group), input_(input) {
    this->RegisterMe();
...
/// PulseNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S, typename E> class PulseNode : public EventNode<S> {
public:
  PulseNode(const Group &group, const State<S> &input, const Event<E> &trigger)
      : PulseNode::EventNode(group), input_(input), trigger_(trigger) {
    this->RegisterMe();
    this->AttachToMe(GetInternals(input).GetNodeId());
    this->AttachToMe(GetInternals(trigger).GetNodeId());
  }
...
public:
  FlattenStateNode(const Group &group, const State<TState<S>> &outer)
      : FlattenStateNode::StateNode(
            group, GetInternals(GetInternals(outer).Value()).Value()),
        outer_(outer), inner_(GetInternals(outer).Value()) {
    this->RegisterMe();
...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <template <typename...> class TList, template <typename> class TState,
          typename V, typename... TParams>
...
public:
  ...
  FlattenStateListNode(const Group &group, const State<InputListType> &outer)
      : FlattenStateListNode::StateNode(
            group, MakeFlatList(GetInternals(outer).Value())),
        outer_(outer), inner_(GetInternals(outer).Value()) {
    this->RegisterMe();
    this->AttachToMe(GetInternals(outer_).GetNodeId());
  ...
  virtual UpdateResult Update(TurnId turnId) noexcept override {
    ...
    // Check if there's a new inner node.
    if (!(std::equal(begin(newInner), end(newInner), begin(inner_),
                     end(inner_)))) {
      for (const State<V> &state : inner_)
        this->DetachFromMe(GetInternals(state).GetNodeId());
    ...
    if (!(std::equal(
            begin(newValue), end(newValue), begin(curValue), end(curValue),
            [](const auto &a, const auto &b) { return !HasChanged(a, b); }))) {
      this->Value() = std::move(newValue);
      return UpdateResult::changed;
    }
...
/// FlattenStateListNode
///////////////////////////////////////////////////////////////////////////////////////////////////
template <template <typename...> class TMap, template <typename> class TState,
          typename K, typename V, typename... TParams>
...
public:
  ...
  FlattenStateMapNode(const Group &group, const State<InputMapType> &outer)
      : FlattenStateMapNode::StateNode(
            group, MakeFlatMap(GetInternals(outer).Value())),
        outer_(outer), inner_(GetInternals(outer).Value()) {
    this->RegisterMe();
    this->AttachToMe(GetInternals(outer_).GetNodeId());
  ...
  virtual UpdateResult Update(TurnId turnId) noexcept override {
    ...
    if (!(std::equal(begin(newValue), end(newValue), begin(curValue),
                     end(curValue), [](const auto &a, const auto &b) {
                       return !HasChanged(a.first, b.first) &&
                              !HasChanged(a.second, b.second);
                     }))) {
      this->Value() = std::move(newValue);
      return UpdateResult::changed;
    }
...
private:
  static FlatMapType MakeFlatMap(const InputMapType &map) {
    ...
    for (const auto &entry : map)
      MapInsert(res, typename FlatMapType::value_type{
                         entry.first, GetInternals(entry.second).Value()});
    return res;
  }
...
public:
  FlattenObjectNode(const Group &group, const State<T> &obj)
      : StateNode<TFlat>(in_place, group, GetInternals(obj).Value(),
                         FlattenedInitTag{}),
        obj_(obj) {
    this->RegisterMe();
  ...
  virtual UpdateResult Update(TurnId turnId) noexcept override {
    ...
    if (HasChanged(newValue, static_cast<const T &>(this->Value()))) {
      ...
      // Steal array from old value for new value so we don't have to
      // re-allocate. The old value will freed after the assignment.
      this->Value().memberIds_.clear();
      this->Value() = TFlat{newValue, FlattenedInitTag{},
                            std::move(this->Value().memberIds_)};
...
auto Hold(const Group &group, T &&initialValue, const Event<E> &evnt)
    -> State<E> {
  using ::react::impl::CreateWrappedNode;
...
template <typename S> auto Monitor(const State<S> &state) -> Event<S> {
...
auto Iterate(const Group &group, T &&initialValue, F &&func,
             const Event<E> &evnt) -> State<S> {
  using ::react::impl::CreateWrappedNode;
  ...
  return CreateWrappedNode<State<S>, IterateNode<S, FuncType, E>>(
      group, std::forward<T>(initialValue), std::forward<F>(func),
      SameGroupOrLink(group, evnt));
}

template <typename S, typename T, typename F, typename E>
auto IterateByRef(const Group &group, T &&initialValue, F &&func,
                  const Event<E> &evnt) -> State<S> {
  using ::react::impl::CreateWrappedNode;
...
auto Iterate(T &&initialValue, F &&func, const Event<E> &evnt) -> State<S> {
  return Iterate<S>(evnt.GetGroup(), std::forward<T>(initialValue),
                    std::forward<F>(func), evnt);
}

template <typename S, typename T, typename F, typename E>
auto IterateByRef(T &&initialValue, F &&func, const Event<E> &evnt)
    -> State<S> {
  return IterateByRef<S>(evnt.GetGroup(), std::forward<T>(initialValue),
                         std::forward<F>(func), evnt);
}
...
auto Iterate(const Group &group, T &&initialValue, F &&func,
             const Event<E> &evnt, const State<Us> &...states) -> State<S> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::SameGroupOrLink;
  ...
  return CreateWrappedNode<State<S>, SyncedIterateNode<S, FuncType, E, Us...>>(
      group, std::forward<T>(initialValue), std::forward<F>(func),
      SameGroupOrLink(group, evnt), SameGroupOrLink(group, states)...);
}

template <typename S, typename T, typename F, typename E, typename... Us>
auto IterateByRef(const Group &group, T &&initialValue, F &&func,
                  const Event<E> &evnt, const State<Us> &...states)
    -> State<S> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::SameGroupOrLink;
  ...
  return CreateWrappedNode<State<S>,
                           SyncedIterateByRefNode<S, FuncType, E, Us...>>(
      group, std::forward<T>(initialValue), std::forward<F>(func),
      SameGroupOrLink(group, evnt), SameGroupOrLink(group, states)...);
}

template <typename S, typename T, typename F, typename E, typename... Us>
auto Iterate(T &&initialValue, F &&func, const Event<E> &evnt,
             const State<Us> &...states) -> State<S> {
  return Iterate<S>(evnt.GetGroup(), std::forward<T>(initialValue),
                    std::forward<F>(func), evnt, states...);
}
...
auto Snapshot(const Group &group, const State<S> &state, const Event<E> &evnt)
    -> State<S> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::SameGroupOrLink;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename S, typename E>
auto Pulse(const Group &group, const State<S> &state, const Event<E> &evnt)
    -> Event<S> {
  using ::react::impl::CreateWrappedNode;
...
auto Flatten(const Group &group, const State<TState<S>> &state) -> State<S> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::FlattenStateNode;
  using ::react::impl::SameGroupOrLink;

  return CreateWrappedNode<State<S>, FlattenStateNode<S, TState>>(
      group, SameGroupOrLink(group, state));
}
...
auto FlattenList(const Group &group,
                 const State<TList<TState<V>, TParams...>> &list)
    -> State<TList<V>> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::FlattenStateListNode;
  using ::react::impl::SameGroupOrLink;

  return CreateWrappedNode<State<TList<V>>,
                           FlattenStateListNode<TList, TState, V, TParams...>>(
...
template <template <typename...> class TList, template <typename> class TState,
          typename V, typename... TParams,
          typename = std::enable_if_t<std::is_base_of_v<State<V>, TState<V>>>>
auto FlattenList(const State<TList<TState<V>, TParams...>> &list)
    -> State<TList<V>> {
...
template <template <typename...> class TMap, template <typename> class TState,
          typename K, typename V, typename... TParams,
          typename = std::enable_if_t<std::is_base_of_v<State<V>, TState<V>>>>
auto FlattenMap(const Group &group,
                const State<TMap<K, TState<V>, TParams...>> &map)
    -> State<TMap<K, V>> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::FlattenStateMapNode;
  using ::react::impl::SameGroupOrLink;

  return CreateWrappedNode<State<TMap<K, V>>,
                           FlattenStateMapNode<TMap, TState, K, V, TParams...>>(
...
template <template <typename...> class TMap, template <typename> class TState,
          typename K, typename V, typename... TParams,
          typename = std::enable_if_t<std::is_base_of_v<State<V>, TState<V>>>>
auto FlattenMap(const State<TMap<K, TState<V>, TParams...>> &map)
    -> State<TMap<K, V>> {
...
/// Flattened
///////////////////////////////////////////////////////////////////////////////////////////////////
template <typename C> class Flattened : public C {
public:
  using C::C;

  Flattened(const C &base) : C(base) {}

  Flattened(const C &base, ::react::impl::FlattenedInitTag)
      : C(base), initMode_(true) {}

  Flattened(const C &base, ::react::impl::FlattenedInitTag,
            std::vector<::react::impl::NodeId> &&emptyMemberIds)
      : C(base), initMode_(true),
        memberIds_(
            std::move(emptyMemberIds)) // This will be empty, but has
                                       // pre-allocated storage. It's a tweak.
  {}

  template <typename T> Ref<T> Flatten(State<T> &signal) {
    if (initMode_)
...
private:
  ...
  std::vector<::react::impl::NodeId> memberIds_;

  template <typename T, typename TFlat> friend class impl::FlattenObjectNode;
};
...
auto FlattenObject(const Group &group, const State<T> &obj) -> State<TFlat> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::FlattenObjectNode;
  using ::react::impl::SameGroupOrLink;

  return CreateWrappedNode<State<TFlat>, FlattenObjectNode<T, TFlat>>(group,
                                                                      obj);
}
...
template <typename T, typename TFlat = typename T::Flat>
auto FlattenObject(const Group &group, const State<Ref<T>> &obj)
    -> State<TFlat> {
  using ::react::impl::CreateWrappedNode;
  using ::react::impl::FlattenObjectNode;
  using ::react::impl::SameGroupOrLink;

  return CreateWrappedNode<State<TFlat>, FlattenObjectNode<Ref<T>, TFlat>>(
      group, obj);
}
...
template <typename E> class EventStreamNode;
...
public:
  explicit ObserverNode(const Group &group) : ObserverNode::NodeBase(group) {}
};
...
public:
  ...
  StateObserverNode(const Group &group, FIn &&func, const State<TDeps> &...deps)
      : StateObserverNode::ObserverNode(group), func_(std::forward<FIn>(func)),
        depHolder_(deps...) {
    ...
    react::impl::apply(
        [this](const auto &...deps) {
          this->func_(GetInternals(deps).Value()...);
        },
        depHolder_);
  }
  ...
  ~StateObserverNode() {
    react::impl::apply(
        [this](const auto &...deps) {
          REACT_EXPAND_PACK(this->DetachFromMe(GetInternals(deps).GetNodeId()));
        },
        depHolder_);
    this->UnregisterMe();
  }
...
public:
  ...
  EventObserverNode(const Group &group, FIn &&func, const Event<E> &subject)
      : EventObserverNode::ObserverNode(group), func_(std::forward<FIn>(func)),
        subject_(subject) {
    this->RegisterMe(NodeCategory::output);
...
public:
  template <typename FIn>
  SyncedEventObserverNode(const Group &group, FIn &&func,
                          const Event<E> &subject,
                          const State<TSyncs> &...syncs)
      : SyncedEventObserverNode::ObserverNode(group),
        func_(std::forward<FIn>(func)), subject_(subject),
        syncHolder_(syncs...) {
    this->RegisterMe(NodeCategory::output);
    this->AttachToMe(GetInternals(subject).GetNodeId());
    REACT_EXPAND_PACK(this->AttachToMe(GetInternals(syncs).GetNodeId()));
  ...
  virtual UpdateResult Update(TurnId turnId) noexcept override {
    ...
    apply(
        [this](const auto &...syncs) {
          func_(GetInternals(this->subject_).Events(),
                GetInternals(syncs).Value()...);
        },
        syncHolder_);
...
public:
  ...
  auto GetNodePtr() -> std::shared_ptr<ObserverNode> & { return nodePtr_; }
...
/// ObserverBase
///////////////////////////////////////////////////////////////////////////////////////////////////
class Observer : protected ::react::impl::ObserverInternals {
private:
  using NodeType = ::react::impl::ObserverNode;

public:
  // Construct state observer with explicit group
  template <typename F, typename T1, typename... Ts>
  static Observer Create(const Group &group, F &&func,
                         const State<T1> &subject1,
                         const State<Ts> &...subjects) {
    return Observer(CreateStateObserverNode(group, std::forward<F>(func),
                                            subject1, subjects...));
  }

  // Construct state observer with implicit group
  template <typename F, typename T1, typename... Ts>
  static Observer Create(F &&func, const State<T1> &subject1,
                         const State<Ts> &...subjects) {
    return Observer(CreateStateObserverNode(
        subject1.GetGroup(), std::forward<F>(func), subject1, subjects...));
  }

  // Construct event observer with explicit group
  template <typename F, typename T>
  static Observer Create(const Group &group, F &&func,
                         const Event<T> &subject) {
    return Observer(
        CreateEventObserverNode(group, std::forward<F>(func), subject));
  }

  // Construct event observer with implicit group
  template <typename F, typename T>
  static Observer Create(F &&func, const Event<T> &subject) {
    return Observer(CreateEventObserverNode(subject.GetGroup(),
                                            std::forward<F>(func), subject));
  }
  ...
  static Observer Create(const Group &group, F &&func, const Event<T> &subject,
                         const State<Us> &...states) {
    return Observer(CreateSyncedEventObserverNode(group, std::forward<F>(func),
                                                  subject, states...));
  }
  ...
  static Observer Create(F &&func, const Event<T> &subject,
                         const State<Us> &...states) {
    return Observer(CreateSyncedEventObserverNode(
        subject.GetGroup(), std::forward<F>(func), subject, states...));
  }
...
private:
  template <typename F, typename T1, typename... Ts>
  static auto CreateStateObserverNode(const Group &group, F &&func,
                                      const State<T1> &dep1,
                                      const State<Ts> &...deps)
      -> decltype(auto) {
    using ::react::impl::StateObserverNode;
    return std::make_shared<
        StateObserverNode<typename std::decay<F>::type, T1, Ts...>>(
        group, std::forward<F>(func), SameGroupOrLink(group, dep1),
        SameGroupOrLink(group, deps)...);
  }

  template <typename F, typename T>
  static auto CreateEventObserverNode(const Group &group, F &&func,
                                      const Event<T> &dep) -> decltype(auto) {
  ...
  template <typename F, typename T, typename... Us>
  static auto CreateSyncedEventObserverNode(const Group &group, F &&func,
                                            const Event<T> &dep,
                                            const State<Us> &...syncs)
      -> decltype(auto) {
    using ::react::impl::SyncedEventObserverNode;
    return std::make_shared<
        SyncedEventObserverNode<typename std::decay<F>::type, T, Us...>>(
        group, std::forward<F>(func), SameGroupOrLink(group, dep),
        SameGroupOrLink(group, syncs)...);
  }
...
inline NodeId ReactGraph::RegisterNode(IReactNode *nodePtr,
...
inline void ReactGraph::AddSyncPointDependency(SyncPoint::Dependency dep,
                                               bool syncLinked) {
  if (syncLinked)
...
inline void ReactGraph::Propagate() {
  ...
  while (scheduledNodes_.FetchNext()) {
    for (NodeId nodeId : scheduledNodes_.Next()) {
      ...
      // Special handling for link output nodes. They have no successors and
...
inline bool ReactGraph::TopoQueue::FetchNext() {
  ...
  auto p =
      std::partition(queueData_.begin(), queueData_.end(),
                     [t = minLevel_](const Entry &e) { return t != e.second; });
...
inline size_t TransactionQueue::ProcessNextBatch() {
  ...
  for (;;) {
    if (!skipPop) {
      ...
      syncLinked =
          IsBitmaskSet(curTransaction.flags, TransactionFlags::sync_linked);
    ...
    graph_.DoTransaction([&] {
      ...
      if (canMerge) {
        ...
        for (;;) {
          ...
          canMerge = IsBitmaskSet(curTransaction.flags,
